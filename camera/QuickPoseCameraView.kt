...
class QuickPoseCameraView(
    ...
    val cameraFrameSize = Size(960, 1280)
    ...
    suspend fun start() {
        ...
        startCamera(
                onComplete = { inputTexture, cameraFrameSize, upscale ->
                    quickPose.onCameraStarted(isFrontCamera, cameraFrameSize, upscale)
    ...
    private suspend fun startCamera(
        ...
        val cameraViewSize = this.holder.getSurfaceFrame() // rect is the visible view area.
        val upscale =
                max(
                        cameraViewSize.width().toFloat() / cameraFrameSize.width.toFloat(),
                        cameraViewSize.height().toFloat() / cameraFrameSize.height.toFloat()
                )

        this.layoutParams =
                FrameLayout.LayoutParams(
                        (cameraFrameSize.width * upscale).toInt(),
                        (cameraFrameSize.height * upscale).toInt(),
                        Gravity.CENTER
                )
        this.camera = camera
            ...
            val captureSessionCallback =
                    object : CameraCaptureSession.StateCallback() {
                        ...
                        override fun onConfigured(session: CameraCaptureSession) {
                            ...
                            // Detach the SurfaceTexture from the GL context we created
                            // earlier so that
                            // the MediaPipe pipeline can attach it.
                            // Only needed if MediaPipe pipeline doesn't provide a
                            // SurfaceTexture.
                            mlTexture.detachFromGLContext()

                            mainThreadExecutor.execute {
                                onComplete?.invoke(mlTexture, cameraFrameSize, upscale)
                            }
                        }
        ...
        manager.openCamera(
                ...
                object : CameraDevice.StateCallback() {
                    ...
                    override fun onDisconnected(device: CameraDevice) {
                        Log.w(QuickPoseCameraView.TAG, "Camera $cameraId has been disconnected")
                    }
    ...
    private fun chooseOptimalSize(choices: Array<Size>?, targetResolution: Size): Size {
        ...
        return choices.filter { it.height.toFloat() / it.width == targetRatio }.minByOrNull {
